...
public class JBuffer extends JCompType {
  ...
  class JavaBuffer extends JavaCompType {
    ...
    @Override
    void genJSON(CodeBuffer cb, String fname) throws java.io.IOException {
      cb.append("if (" + fname + " == null || " + fname +".getCount() ==0) {\n");
      cb.append("  writer.name(\""+fname+"\").value(\"\");\n");
      cb.append("}\n");
      cb.append("else {\n");
      cb.append("  writer.name(\""+fname+"\").value("+fname+".toString());\n");
      cb.append("}\n");
    }
    ...
    void genGetSet(CodeBuffer cb, String fname,boolean trackDirtyFields) {
      ...
      cb.append("}\n");
      cb.append("public void set"+toCamelCase(fname)+"( "+getType()+" "+fname+") {\n");
      if (trackDirtyFields) { 
      ...
      cb.append("this."+fname+"= new FlexBuffer("+fname+".get(),0,"+fname+".getCount());\n");
      cb.append("}\n");
