...
/**
 * 
 * The workhorse of the library. This class (currently) implements SpillWriter
 * and thus can accept a very large number of <<unsorted>> Key/Value pairs,
 * which it then sorts in batches and stores in a temp directory, and finally
 * merges everything back into one sorted output via a merge sort operation.
 * 
 * This class implements two constructors, one that takes a RawComparator and
 * one that takes an OptimizedKeyGeneratorAndComparator instance. The former is
 * used to compare Raw Key/Value pairs, while the later is used to create a an
 * optimized key representation which can then be used for sorting within the
 * segment as well as during the final merge sort.
 * 
 ...
 * 
 * @param <KeyType>
 *          WritableComparable Type you will be using to represent a key
 * @param <ValueType>
 *          Writable Type you will be using to represent a value
 */
...
public class MergeSortSpillWriter<KeyType extends WritableComparable, ValueType extends Writable> implements
  ...
  private static int DEFAULT_SPILL_INDEX_BUFFER_SIZE = 1000000; // 100K * 4 =
                                                                // 40000K .. OR
                                                                // 40MB //
                                                                // memory usage
  private static int SPILL_DATA_ITEM_AVG_SIZE = 400;
  private static int DEFAULT_SPILL_DATA_BUFFER_SIZE = DEFAULT_SPILL_INDEX_BUFFER_SIZE * SPILL_DATA_ITEM_AVG_SIZE; // 1
                                                                                                                  // *10(6)
                                                                                                                  // *
                                                                                                                  // 4
                                                                                                                  // *
  ...
  // the size of the data buffer used to accumulate key value pairs during the
  // sort / spill process
  ...
  private static void swap(int x[], int a, int b) {
    // LOG.info("Swapping [" + a + "][" + _spillDataBuffer.getLong(x[a])
    // +"] and [" + b + "][" + _spillDataBuffer.getLong(x[b]) + "]");
    int t = x[a];
    x[a] = x[b];
    x[b] = t;
  }
  ...
  /**
   * use this constructor for creating a merger that uses an optimized key
   * generator
   * 
   * 
   * @param conf
  ...
  public MergeSortSpillWriter(Configuration conf, RawDataSpillWriter<KeyType, ValueType> outputSpillWriter,
      ...
      Class<ValueType> valueClass, boolean compressOutput, Reporter reporter) throws IOException {

    init(conf, outputSpillWriter, tempDataFileSystem, tempDirectoryPath, null, keyGenerator, null, keyClass,
        valueClass, compressOutput, reporter);
  }
   ...
   */
  public MergeSortSpillWriter(Configuration conf, RawDataSpillWriter<KeyType, ValueType> outputSpillWriter,
      FileSystem tempDataFileSystem, Path tempDirectoryPath, SpillValueCombiner<KeyType, ValueType> optionalCombiner,
      RawKeyValueComparator<KeyType, ValueType> comparator, Class<KeyType> keyClass, Class<ValueType> valueClass,
      boolean compressOutput, Reporter reporter) throws IOException {

    init(conf, outputSpillWriter, tempDataFileSystem, tempDirectoryPath, comparator, null, optionalCombiner, keyClass,
        valueClass, compressOutput, reporter);
  }
  ...
  private int compareUsingOptimizedRawBufferValues(byte[] dataAsBytes, int lValueDataOffset, int rValueDataOffset)
    ...
    return _optimizedKeyGenerator.compareOptimizedBufferKeys(dataAsBytes, lValueDataOffset + buffer1Offset, buffer1Len,
  ...
  private void init(Configuration conf, RawDataSpillWriter<KeyType, ValueType> outputSpillWriter,
      FileSystem tempDataFileSystem, Path tempDirectoryPath, RawKeyValueComparator<KeyType, ValueType> comparator,
  ...
  private int med3UsingOptimizedComparatorWithBuffer(byte[] spillData, int x[], int a, int b, int c)
    ...
    ((compareUsingOptimizedRawBufferValues(spillData, x[b], x[c]) < 0) ? b : (compareUsingOptimizedRawBufferValues(
        ...
        ((compareUsingOptimizedRawBufferValues(spillData, x[b], x[c]) > 0) ? b : (compareUsingOptimizedRawBufferValues(
            spillData, x[a], x[c]) > 0) ? c : a)); // (x[b] > x[c] ? b : x[a] >
                                                   // x[c] ? c : a));
  ...
  private int med3UsingOptimizedComparatorWithLongsAndBuffer(byte[] spillData, int x[], int a, int b, int c)
    ...
    ((compareUsingOptimizedRawLongAndBufferValues(spillData, x[b], x[c]) < 0) ? b
        ...
        : // (x[b] < x[c] ? b : x[a] < x[c] ? c : a) :
        ((compareUsingOptimizedRawLongAndBufferValues(spillData, x[b], x[c]) > 0) ? b
            : (compareUsingOptimizedRawLongAndBufferValues(spillData, x[a], x[c]) > 0) ? c : a)); // (x[b]
                                                                                                  // >
                                                                                                  // x[c]
                                                                                                  // ?
                                                                                                  // b
  ...
  private int med3UsingRawComparator(byte[] spillData, int x[], int a, int b, int c) throws IOException {
    ...
    return (compareUsingRawComparator(_comparator, spillData, x[a], spillData, x[b]) < 0 ? // x[a]
                                                                                           ...
                                                                                           // x[b]
                                                                                           // ?
    ((compareUsingRawComparator(_comparator, spillData, x[b], spillData, x[c]) < 0) ? b : (compareUsingRawComparator(
        _comparator, spillData, x[a], spillData, x[c]) < 0) ? c : a)
        : // (x[b] < x[c] ? b : x[a] < x[c] ? c : a) :
        ((compareUsingRawComparator(_comparator, spillData, x[b], spillData, x[c]) > 0) ? b
            : (compareUsingRawComparator(_comparator, spillData, x[a], spillData, x[c]) > 0) ? c : a)); // (x[b]
                                                                                                        // >
                                                                                                        // x[c]
                                                                                                        // ?
                                                                                                        // b
  ...
  private void sortAndSpill(RawDataSpillWriter<KeyType, ValueType> optionalWriter) throws IOException {
    ...
    // figure out if we are writing to temporary spill writer or passed in spill
    // writer ...
    ...
    if (optionalWriter != null) {
      ...
      spillWriter = new SequenceFileSpillWriter<KeyType, ValueType>(_tempDataFileSystem, _conf, spillFilePath,
          _keyClass, _valueClass, null, _compressOutput);
    }
    ...
    try {
      ...
      for (i = 0; i < _spillItemCount; ++i) {
        try {
          ...
          // if optimized key ... we need to write optimized key value as well
          // as regular key value ...
          if (_optimizedKeyGenerator != null) {
          ...
          // LOG.info("Spilling Raw Record: startPos:" + dataOffset +
          // " optKeyBufferSize:" + optimizedBufferLen + " keySize:" + keyLen +
          // " keyDataPos:" + keyPos+ " valueSize:" + valueLen + " valuePos:" +
          // valuePosition);

          // now write this out to the sequence file ...

          // if direct spill, we don't write lengths and optimized bits to
          // stream ...
          ...
          // otherwise ... in the optimized key case ...
            ...
            spillWriter.spillRawRecord(bufferAsBytes, dataOffset, keyLen + _optimizedKey.getHeaderSize() + 4,
                bufferAsBytes, valuePosition, valueLen + optimizedBufferLen);
          }
          // increment progress...
          if (i % 100000 == 0) {
          ...
          LOG.error("Error in Iteration. " + " DataOffset:" + dataOffset + " index:" + i + " keyLen:" + keyLen
              + " keyPos:" + keyPos + " valueLen:" + valueLen + " valuePos:" + valuePosition + " spillDataBufferSize:"
  ...
  private void sortUsingOptimizedBufferKeys(byte[] dataBytes, int x[], int off, int len) throws IOException {
    ...
    if (len > 7) {
      ...
      m = med3UsingOptimizedComparatorWithBuffer(dataBytes, x, l, m, n); // for
                                                                         // mid-sized
                                                                         // arrays,
                                                                         // take
                                                                         // median
  ...
  private void sortUsingOptimizedLongAndBufferKeys(byte[] dataBytes, int x[], int off, int len) throws IOException {
    ...
    if (len > 7) {
      ...
      m = med3UsingOptimizedComparatorWithLongsAndBuffer(dataBytes, x, l, m, n); // for
                                                                                 // mid-sized
                                                                                 // arrays,
                                                                                 // take
                                                                                 // median
  ...
  private void sortUsingOptimizedLongComparator(int x[], int off, int len) throws IOException {
    ...
    if (len > 7) {
      ...
      m = med3UsingOptimizedComparatorWithLongs(x, l, m, n); // Mid-size, med of
                                                             // 3
    }
   ...
   */
  /*
   * //DISABLE THIS CODE UNTIL WE CAN DEVELOP A UNIT TEST TO VALIDATE IT public
   * void spillRawRecord(DataInputStream keyStream,DataInputStream valueStream)
   * throws IOException { // if index is full , trigger merge as well if
   * (_spillItemCount == _spillIndexBufferSize) {
   * //LOG.info("Spill Item Count == " + SPILL_INDEX_BUFFER_SIZE +
   * ". Flushing"); sortAndSpill(null); }
   * 
   * 
   * boolean done = false;
   * 
   * // ok, writing from a stream is a little trickier, since we could overflow
   * at any moment // and (in the case of TFile) in some cases the input stream
   * is not rewindable boolean wroteKey = false; int totalKeyBytesWritten = 0;
   * int keySizePos = 0; boolean wroteValue = false; int totalValueBytesWritten
   * = 0; int valueSizePos = 0;
   * 
   * 
   * while (!done) {
   * 
   * // mark buffer position ... int startPositon = _spillDataBuffer.position();
   * //LOG.info("Buffer start position:" + startPositon);
   * 
   * boolean overflow = false; try { int optimizedKeyBufferSize = 0; // if
   * optimized comparator is available ... if (_optimizedKeyGenerator != null) {
   * // we don't have access to key, value bits yet, so leave space for header
   * bytes _spillDataBuffer.position(startPositon +
   * OptimizedKey.FIXED_HEADER_SIZE); } // save key size position keySizePos =
   * _spillDataBuffer.position(); // LOG.info("keySizePos:" + keySizePos);
   * 
   * // skip past key length _spillDataBuffer.position(keySizePos + 4); // and
   * skip past key bytes already written (via overflow condition)..
   * _spillDataBuffer.position(_spillDataBuffer.position() +
   * totalKeyBytesWritten);
   * 
   * // now if we did not finish writing key ... if (!wroteKey) { // write key
   * into remaining bytes ... int bufferBytesAvailable =
   * _spillDataBuffer.remaining(); int keyBytesWritten =
   * keyStream.read(_spillDataBufferBytes, _spillDataBuffer.position(),
   * bufferBytesAvailable); totalKeyBytesWritten += keyBytesWritten; if
   * (bufferBytesAvailable == keyBytesWritten) { // ok we reached an overflow
   * condition !!! throw new BufferOverflowException(); } else { // advance
   * cursor _spillDataBuffer.position(_spillDataBuffer.position() +
   * keyBytesWritten); // ok we are done writing key ... wroteKey = true; } } //
   * now save value size position valueSizePos = _spillDataBuffer.position(); //
   * and calculate key size int keySize = valueSizePos - keySizePos - 4; //
   * reseek back _spillDataBuffer.position(keySizePos); // write out real key
   * size _spillDataBuffer.putInt(keySize); // skip to value size position + 4
   * _spillDataBuffer.position(valueSizePos + 4); // and skip past already
   * written data (via overflow condition)
   * _spillDataBuffer.position(_spillDataBuffer.position() +
   * totalValueBytesWritten); // now if we did not finish writing value ... if
   * (!wroteValue) { // write key into remaining bytes ... int
   * bufferBytesAvailable = _spillDataBuffer.remaining(); int valueBytesWritten
   * = valueStream.read(_spillDataBufferBytes, _spillDataBuffer.position(),
   * bufferBytesAvailable); totalValueBytesWritten += valueBytesWritten; if
   * (bufferBytesAvailable == valueBytesWritten) { // ok we reached an overflow
   * condition !!! throw new BufferOverflowException(); } else { // advance
   * cursor _spillDataBuffer.position(_spillDataBuffer.position() +
   * valueBytesWritten); // ok we are done writing value bytes ... wroteValue =
   * true; } } // save end position int endPosition =
   * _spillDataBuffer.position(); // calculate value size int valueSize =
   * endPosition - valueSizePos - 4; // reseek back to value size pos
   * _spillDataBuffer.position(valueSizePos); // write value size
   * _spillDataBuffer.putInt(valueSize); // ok now ... if optimized key
   * generation is required ... if (_optimizedKeyGenerator != null) { // seek
   * back to start position _spillDataBuffer.position(startPositon); // gen
   * optimized key _optimizedKeyGenerator.generateOptimizedKeyForRawPair(
   * _spillDataBufferBytes, keySizePos+4, totalKeyBytesWritten,
   * _spillDataBufferBytes, valueSizePos+4, totalValueBytesWritten,
   * _optimizedKey); // write out optimized key header // and calc trailing key
   * buffer size optimizedKeyBufferSize =
   * _optimizedKey.writeHeaderToStream(_outputStream); } // seek forward to end
   * position _spillDataBuffer.position(endPosition); // and now if there is
   * optional optimized key buffer data ... // append it at end if
   * (optimizedKeyBufferSize != 0) {
   * _optimizedKey.writeBufferToStream(_outputStream); } // store start position
   * in index buffer _spillIndexBuffer[_spillItemCount] = startPositon; //
   * increment ... _spillItemCount++;
   * 
   * //LOG.info("startPos:" + startPositon + " optKeySize:" + optimizedKeySize +
   * " keySizePos:" + keySizePos + " keySize:" + keySize + " valueSizePos:" +
   * valueSizePos + " valueSize:" + valueSize);
   * 
   * done = true; } // trap for buffer overflow specifically catch
   * (IllegalArgumentException e) {//LOG.info(
   * "IllegalArgumentException - Buffer Overflow detected while writing to Spill Data Buffer. Flushing"
   * ); overflow = true; } catch (BufferOverflowException e) {//LOG.info(
   * "BufferOverflowException - Buffer Overflow detected while writing to Spill Data Buffer. Flushing"
   * ); overflow = true; } if (overflow) { // if overflow happened with start
   * position at zero... this is bad news. // it means that key + value +
   * optimized bits > spill data buffer size! if (startPositon == 0) { throw new
   * IOException("FATAL: KeySize:" + totalKeyBytesWritten + " + ValueSize:" +
   * totalValueBytesWritten + " GT SpillBufferSize:" +
   * _spillDataBuffer.capacity()); } // otherwise .. // overflow in the stream
   * case is a little tricky ... // first rewind buffer position ...
   * _spillDataBuffer.position(startPositon); // sort and spill whatever we did
   * fit into the buffer ... sortAndSpill(null); // now, we need to reconstitute
   * the buffer back to previous state if (totalKeyBytesWritten != 0) { //
   * calculate offset ... int newKeyBytesOffset = 4; // if optimized .. if
   * (_optimizedKeyGenerator != null) { newKeyBytesOffset +=
   * OptimizedKey.FIXED_HEADER_SIZE; } // copy bits
   * System.arraycopy(_spillDataBufferBytes, keySizePos +
   * 4,_spillDataBufferBytes, newKeyBytesOffset, totalKeyBytesWritten);
   * 
   * // and copy value bits potentially if (totalValueBytesWritten != 0) { int
   * newValueBytesOffset = newKeyBytesOffset + totalKeyBytesWritten + 4; // copy
   * bits System.arraycopy(_spillDataBufferBytes, valueSizePos +
   * 4,_spillDataBufferBytes, newValueBytesOffset, totalValueBytesWritten); } }
   * 
   * } } }
   */
  ...
  private void sortUsingRawComparator(byte[] dataBytes, int x[], int off, int len) throws IOException {
    ...
    if (len > 7) {
      ...
      m = med3UsingRawComparator(dataBytes, x, l, m, n); // for mid-sized
                                                         // arrays, take median
                                                         // of 3
    }
    int vOffset = x[m];
  ...
  public void spillRecord(KeyType key, ValueType value) throws IOException {
    ...
    while (!done) {
      ...
      try {
        ...
        // LOG.info("startPos:" + startPositon + " optKeyBufferSize:" +
        // optimizedKeyBufferSize + " keySizePos:" + keySizePos + " keySize:" +
        // keySize + " valueSizePos:" + valueSizePos + " valueSize:" +
        // valueSize);
